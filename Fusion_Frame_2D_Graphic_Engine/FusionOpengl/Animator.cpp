...
void FUSIONOPENGL::Animator::UpdateBlendedAnimation(Animation* BaseAnimation, Animation* LayeredAnimation, float blendFactor, float deltaTime)
{
	float a = 1.0f;
...
void FUSIONOPENGL::Animator::CalculateBlendedBoneTransform(Animation* pAnimationBase, const BoneNodeData* node, 
...
void FUSIONOPENGL::Animator::CalculateBoneTransform(Animation* animation, const BoneNodeData* node, const float currentTime, glm::mat4 parentTransform)
{
	std::string nodeName = node->name;
	glm::mat4 nodeTransform = node->transformation;
