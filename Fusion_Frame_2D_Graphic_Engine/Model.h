...
public:
    ...
    void Draw(GLuint shader , Camera3D& camera, GLuint shadowMap , GLuint cube_map_texture)
    {
        UseShaderProgram(shader);
    ...
    void DrawMultipleShadowMaps(GLuint shader, Camera3D& camera, std::function<void()> ShaderPreperation, GLuint cube_map_texture)
    {
        UseShaderProgram(shader);
    ...
    void Draw(GLuint shader, Camera3D& camera, GLuint shadowMap, GLuint cube_map_texture , GLuint SSAOmap)
    {
        UseShaderProgram(shader);
        glUniform1f(glGetUniformLocation(shader, "modelID"), this->GetModelID());
...
private:
    ...
    newwww::Mesh processMesh(aiMesh* mesh, const aiScene* scene)
    {
        
        vector<newwww::Vertex> vertices;
        ...
        newwww::Mesh newmesh(vertices, indices, textures);
