...
void CalculateSmoothNormals(FUSIONCORE::Mesh& mesh)
{
	auto& Vertices = mesh.GetVertices();
	auto& HalfEdges = mesh.GetHalfEdges();
	auto& duplicates = mesh.GetDuplicateVertexMap();
...
void CalculateNormals(FUSIONCORE::Mesh& Mesh)
{
	auto& faces = Mesh.GetFaces();
	auto& vertices = Mesh.GetVertices();
	...
	{
		auto& faceindices = faces[i]->Indices;
		glm::vec3 Normal = GetTriangleNormal(*vertices[faceindices[0]], *vertices[faceindices[1]], *vertices[faceindices[2]]);

		for (auto& faceindex : faceindices)
		{
			vertices[faceindices[0]]->Normal = Normal;
		}
	}
}
...
void ContructVerticesFromFaces(FUSIONCORE::Mesh& Mesh, glm::vec3 ModelOrigin)
{
	...
	for (size_t i = 0; i < faces.size(); i++)
	{
		...
		for (size_t y = 0; y < face->Indices.size(); y++)
		{
			...
			newVertex.Bitangent = existingVertex->Bitangent;
