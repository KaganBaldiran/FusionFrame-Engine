...
void main()
{
   ...
   if(GlobalIndex >= (BIN_COUNT / N)) return;

   SharedData[2 * LocalIndex] = WorkGroupSums[2 * GlobalIndex];
   SharedData[2 * LocalIndex + 1] = WorkGroupSums[2 * GlobalIndex + 1];
   barrier();
   ...
   {
      WorkGroupSums[2 * GlobalIndex] = SharedData[2 * LocalIndex];
      WorkGroupSums[2 * GlobalIndex + 1] = SharedData[2 * LocalIndex + 1];
   }
}